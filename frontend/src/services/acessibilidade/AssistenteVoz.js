  ...
  static async handle(command, context = {}) {
    ...
    try {
      ...
      if (text.includes('home') || text.includes('inicio') || text.includes('principal')) {
        ...
        ServicoNavegacao.navigate('AcessoLivre', { screen: 'Denuncia' });
      }
      else if (text.includes('configuracoes') || text.includes('ajustes')) {
        ServicoVoz.speak('Abrindo configurações');
        ServicoNavegacao.navigate('AcessoLivre', { screen: 'Configuracoes' });
      }
        ...
        ServicoNavegacao.navigate('AcessoLivre', { screen: 'Buscar' });
      }
        ...
        const screen = fullContext.screen || 'Inicio';

        if (screen === 'Inicio') {
          ServicoVoz.speak(
            'Comandos disponíveis na página inicial: inicio, perfil, denunciar, configurações, voltar, sair, ' +
            'estatísticas, destaques, ver todos os locais, atualizar página, ' +
            'buscar seguido do nome do local e ajuda. O que você deseja?'
          );
        } else {
          ServicoVoz.speak(
            'Comandos gerais: inicio para página inicial, perfil para seus dados, ' +
      ...
      ServicoVoz.speak('Não consegui processar este comando. Tente novamente.');
    } finally {
