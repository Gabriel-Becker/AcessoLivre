...
export default function EditarLocalModal({ visible, onClose, local, onSucesso, altoContraste = false }) {
  ...
  const carregarDadosLocal = useCallback(async () => {
    ...
    try {
      const dados = await ServicoLocal.obterLocal(local.idLocal);
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={handleClose}>
      <View style={estilos.overlay}>
        <View style={[estilos.Recipiente, { backgroundColor: t.colors.surface, width: larguraModal }]}> 
          <ScrollView
