...
public class CodigoRecuperacaoSenhaController {

    private final CodigoRecuperacaoSenhaService passwordResetCodeService;
     ...
     */
    @GetMapping
    public ResponseEntity<List<CodigoRecuperacaoSenhaResponseDTO>> listarTodos() {
        try {
            log.info("Recebida requisição para listar todos os códigos de reset de senha");
            List<CodigoRecuperacaoSenha> codigos = passwordResetCodeService.listarTodos();
            List<CodigoRecuperacaoSenhaResponseDTO> responseDTOs = CodigoRecuperacaoSenhaMapper.fromEntityList(codigos);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<CodigoRecuperacaoSenhaResponseDTO> buscarPorId(@PathVariable Long id) {
        try {
            log.info("Recebida requisição para buscar código de reset por ID: {}", id);
            Optional<CodigoRecuperacaoSenha> codigo = passwordResetCodeService.buscarPorId(id);
            
            if (codigo.isPresent()) {
                log.info("Código de reset encontrado com ID: {}", id);
                return ResponseEntity.ok(CodigoRecuperacaoSenhaMapper.toResponse(codigo.get()));
            } else {
     ...
     */
    @PostMapping
    public ResponseEntity<?> salvar(@Valid @RequestBody CodigoRecuperacaoSenhaRequestDTO requestDTO) {
        try {
            log.info("Recebida requisição para salvar novo código de reset de senha");
            CodigoRecuperacaoSenha codigo = passwordResetCodeService.salvar(requestDTO);
            log.info("Código de reset salvo com sucesso. ID: {}", codigo.getId());
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(CodigoRecuperacaoSenhaMapper.toResponse(codigo));
    ...
    public ResponseEntity<Boolean> verificarCodigo(@RequestParam String code) {
        try {
            ...
            boolean ehValido = passwordResetCodeService.codigoEhValido(code);
            log.info("Código válido: {}", ehValido);
            return ResponseEntity.ok(ehValido);
        } catch (Exception e) {
    ...
    public ResponseEntity<List<CodigoRecuperacaoSenhaResponseDTO>> buscarCodigosValidosPorUsuario(@PathVariable Long idUsuario) {
        try {
            log.info("Recebida requisição para buscar códigos válidos do usuário ID: {}", idUsuario);
            List<CodigoRecuperacaoSenha> codigos = passwordResetCodeService.buscarCodigosValidosPorUsuario(idUsuario);
            List<CodigoRecuperacaoSenhaResponseDTO> responseDTOs = CodigoRecuperacaoSenhaMapper.fromEntityList(codigos);
    ...
    public ResponseEntity<List<CodigoRecuperacaoSenhaResponseDTO>> buscarPorUsuario(@PathVariable Long idUsuario) {
        try {
            log.info("Recebida requisição para buscar códigos do usuário ID: {}", idUsuario);
            List<CodigoRecuperacaoSenha> codigos = passwordResetCodeService.buscarPorUsuario(idUsuario);
            List<CodigoRecuperacaoSenhaResponseDTO> responseDTOs = CodigoRecuperacaoSenhaMapper.fromEntityList(codigos);
